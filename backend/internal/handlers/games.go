...
func getGamesBySeason(db *database.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		for rows.Next() {
			...
			var dayOfWeek, location, primetime, network, status, homeConference, homeDivision, homeLogoURL, awayConference, awayDivision, awayLogoURL *string
			...
			err := rows.Scan(
				...
				&homeTeamID, &homeAbbr, &homeCity, &homeName, &homeConference, &homeDivision, &homePrimaryColor, &homeSecondaryColor, &homeLogoURL,
				&awayTeamID, &awayAbbr, &awayCity, &awayName, &awayConference, &awayDivision, &awayPrimaryColor, &awaySecondaryColor, &awayLogoURL,
			)
			if err != nil {
				continue
			}
