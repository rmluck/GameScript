...
func TestCalculateWinPct(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            result := calculateWinPct(tt.wins, tt.losses, tt.ties)
...
func TestCalculateGamesBack(t *testing.T) {
    tests := []struct {
        ...
        {
            name: "Same team",
            leader: TeamRecord{TeamID: 1, Wins: 10, Losses: 5, Ties: 0},
            team:   TeamRecord{TeamID: 1, Wins: 10, Losses: 5, Ties: 0},
            expected: 0.0,
        },
        {
            name: "1 game back",
            leader: TeamRecord{TeamID: 1, Wins: 10, Losses: 5, Ties: 0},
            team:   TeamRecord{TeamID: 2, Wins: 9, Losses: 6, Ties: 0},
            expected: 1.0,
        },
        {
            name: "2.5 games back",
            leader: TeamRecord{TeamID: 1, Wins: 10, Losses: 5, Ties: 0},
            team:   TeamRecord{TeamID: 2, Wins: 8, Losses: 7, Ties: 0},
            expected: 2.0,
        },
        {
            name: "With ties",
            leader: TeamRecord{TeamID: 1, Wins: 10, Losses: 5, Ties: 2},
            team:   TeamRecord{TeamID: 2, Wins: 9, Losses: 6, Ties: 2},
            expected: 1.0,
        },
    }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            result := calculateGamesBack(tt.leader, tt.team)
            if result != tt.expected {
...
func TestGetHeadToHeadWinner(t *testing.T) {
    teamA := TeamRecord{TeamID: 1}
    teamB := TeamRecord{TeamID: 2}
...
func TestFilterByConference(t *testing.T) {
    ...
    afcTeams := filterByConference(teams, "AFC")
    if len(afcTeams) != 2 {
        t.Errorf("Expected 2 AFC teams, got %d", len(afcTeams))
    }

    nfcTeams := filterByConference(teams, "NFC")
    if len(nfcTeams) != 2 {
...
func TestApplyTiebreakers(t *testing.T) {
    ...
    sorted := applyDivisionTiebreakers(teams, []GameResult{})
