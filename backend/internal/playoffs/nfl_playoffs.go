...
type PlayoffGenerator struct {
    db *database.DB
}

func NewPlayoffGenerator(db *database.DB) *PlayoffGenerator {
    return &PlayoffGenerator{db: db}
}

// CheckAndEnablePlayoffs checks if all regular season games are complete/picked
func (pg *PlayoffGenerator) CheckAndEnablePlayoffs(scenarioID int, seasonID int) (bool, error) {
    // Count total regular season games
    var totalGames int
    err := pg.db.Conn.QueryRow(`
        SELECT COUNT(*) 
        FROM games 
        WHERE season_id = $1 AND is_postseason = false
...
// GenerateWildCardRound generates the wild card matchups based on standings
func (pg *PlayoffGenerator) GenerateWildCardRound(scenarioID int, seasonID int, sportID int) error {
    ...
    playoffStateID, err := pg.getOrCreatePlayoffState(scenarioID)
    if err != nil {
        return err
    }
    ...
    err = pg.generateConferenceWildCard(playoffStateID, "AFC", nflStandings.AFC.PlayoffSeeds)
    if err != nil {
        return err
    }

    // Generate NFC Wild Card matchups
    err = pg.generateConferenceWildCard(playoffStateID, "NFC", nflStandings.NFC.PlayoffSeeds)
    if err != nil {
        return err
    }
...
func (pg *PlayoffGenerator) generateConferenceWildCard(playoffStateID int, conference string, seeds []standings.PlayoffSeed) error {
...
// GenerateNextRound generates matchups for the next playoff round
func (pg *PlayoffGenerator) GenerateNextRound(scenarioID int, seasonID int, currentRound int) error {
    playoffStateID, err := pg.getPlayoffStateID(scenarioID)
    if err != nil {
        return err
    }

    // Get winners from current round
    winners, err := pg.getRoundWinners(playoffStateID, currentRound)
    if err != nil {
        return err
    }
    ...
    case RoundDivisional:
        genErr = pg.generateDivisionalRound(playoffStateID, scenarioID, seasonID, winners)
    case RoundConference:
        genErr = pg.generateConferenceChampionships(playoffStateID, winners)
    case RoundSuperBowl:
        genErr = pg.generateSuperBowl(playoffStateID, winners)
    }
...
func (pg *PlayoffGenerator) generateDivisionalRound(playoffStateID int, scenarioID int, seasonID int, wildCardWinners map[string][]TeamSeed) error {
...
func (pg *PlayoffGenerator) generateConferenceChampionships(playoffStateID int, divisionalWinners map[string][]TeamSeed) error {
    for conference, winners := range divisionalWinners {
        ...
        `, playoffStateID, RoundConference, 1, conference,
...
func (pg *PlayoffGenerator) generateSuperBowl(playoffStateID int, conferenceWinners map[string][]TeamSeed) error {
...
func (pg *PlayoffGenerator) getRoundWinners(playoffStateID int, round int) (map[string][]TeamSeed, error) {
    query := `
    ...
    for conf := range winners {
        winners[conf] = sortBySeed(winners[conf])
    }

    return winners, nil
}

func sortBySeed(teams []TeamSeed) []TeamSeed {
    // Simple bubble sort
...
func (pg *PlayoffGenerator) getOrCreatePlayoffState(scenarioID int) (int, error) {
    var id int
    err := pg.db.Conn.QueryRow(`
...
func (pg *PlayoffGenerator) getPlayoffStateID(scenarioID int) (int, error) {
    var id int
    err := pg.db.Conn.QueryRow(`
        SELECT id FROM playoff_states WHERE scenario_id = $1
    `, scenarioID).Scan(&id)
...
// CheckRoundComplete checks if all picks for a round are complete
func (pg *PlayoffGenerator) CheckRoundComplete(scenarioID int, round int) (bool, error) {
    playoffStateID, err := pg.getPlayoffStateID(scenarioID)
    if err != nil {
        return false, err
    }
...
// DeleteSubsequentRounds deletes all playoff matchups after the specified round
func (pg *PlayoffGenerator) DeleteSubsequentRounds(scenarioID int, round int) error {
